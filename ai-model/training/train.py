...
def main():
    if not install_ffmpeg():
        print("FFmpeg installation failed. Exiting training.")
        sys.exit(1)
    ...
    for epoch in tqdm(range(args.epochs), desc="Epochs"):
        ...
        val_loss, val_metric = trainer.evaluate(phase="val")
        ...
        print(json.dumps({
            "metrics": [
                ...
                {"Name": "validation:emotion_precision","Value": val_metric['emotion_precision']},
                {"Name": "validation:emotion_accuracy","Value": val_metric['emotion_accuracy']},
                {"Name": "validation:sentiment_precision","Value": val_metric['sentiment_precision']},
                {"Name": "validation:sentiment_accuracy","Value": val_metric['sentiment_accuracy']}
                ]
        }))
        
        if torch.cuda.is_available():
            memory_used = torch.cuda.max_memory_allocated()/1024**3
            print(f"peak GPU memory allocated: {memory_used:.2f} GB")
